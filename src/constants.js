...
/* ---------- 任务超时（毫秒） ----------
 * 这三条是**前台的等待上限**。服务端判「僵尸任务」的阈值必须比它们小
 * （server/index.mjs 的 JOB_STALE_BY_KIND），否则会出现"用户已经等到超时、
 * 服务端还认为任务在跑"的不一致，而且客户端放弃之后任务还在继续调模型花钱。
 * 另外：锁屏/切后台的时长不计入这里的等待时间，见 hooks/pollJob.js。 */
/** 分析 / 素材：前端 10 分钟兜底（后端 8 分钟先判僵尸） */
export const TIMEOUT_ANALYZE_MS = 10 * 60 * 1000;
/** 自测题（后端 4 分钟先判僵尸） */
export const TIMEOUT_QUIZ_MS = 5 * 60 * 1000;
/** 单张图片识别：超过 3 分钟基本就是照片太糊，直接提示重拍（后端 2.5 分钟先判僵尸） */
