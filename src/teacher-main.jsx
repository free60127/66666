...
function TeacherApp() {
  ...
  const exportCsv = () => {
    if (!detail) return;
    const rows = [['班级', '姓名', '学号', '课文', '课文编号', '分数', '提交时间', '结果链接']];
    for (const student of students) {
      if (!student.subs?.length) rows.push([detail.class.name, student.name, student.studentNo, '', '', '', '', '']);
      for (const sub of student.subs || []) rows.push([detail.class.name, student.name, student.studentNo, sub.title, sub.lesson, sub.score, date(sub.at), new URL(resultUrl(sub.jobId), window.location.href).href]);
    }
  ...
  return <div className="teacher-app">
    ...
    {!account ? <main className="teacher-auth card"><h1>{authMode === 'register' ? '注册教师账号' : '教师登录'}</h1><p>教师注册开放。学生无需注册，在学生端输入邀请码即可加入。</p>
      ...
      <main className="teacher-layout"><aside className="teacher-sidebar card"><h2>我的班级</h2><form onSubmit={createClass} className="teacher-create"><input aria-label="新班级名称" placeholder="如：高一 3 班" maxLength={40} required value={newName} onChange={(e) => setNewName(e.target.value)} /><button disabled={busy}>建班</button></form>
        ...
        <div className="card teacher-class-head"><div><h1>{detail.class.name}</h1><p>{detail.class.archivedAt ? '已归档' : `邀请码 ${detail.class.inviteCode} · 学生在「更多 → 加入教师班级」输入`} · {students.length} 名学生</p></div><div className="teacher-actions"><button onClick={refresh} disabled={busy}>刷新成绩</button>{!detail.class.archivedAt && <button onClick={() => copyCode(detail.class.inviteCode)}>复制邀请码</button>}<button onClick={exportCsv}>导出 CSV</button>{!detail.class.archivedAt && <button className="teacher-danger" onClick={() => archive(detail.class)}>归档</button>}</div></div>
        <div className="card"><h2>成绩总览</h2><p className="teacher-muted">按学生与课文显示最近一次成绩。点击分数查看完整批改。</p><div className="teacher-scroll"><table><thead><tr><th>姓名</th><th>学号</th><th>加入时间</th><th>最近活跃</th>{lessons.map((lesson) => <th key={lesson}>{lesson}</th>)}</tr></thead><tbody>{students.map((student) => <tr key={student.studentNo}><td>{student.name}</td><td>{student.studentNo}</td><td>{date(student.joinedAt)}</td><td>{date(student.lastActiveAt)}</td>{lessons.map((lesson) => { const sub = latest(student, lesson); return <td key={lesson}>{sub ? <a href={resultUrl(sub.jobId)} target="_blank" rel="noreferrer" title={`${date(sub.at)} · 查看完整批改`}>{sub.score ?? '查看'}</a> : '—'}</td>; })}</tr>)}</tbody></table></div>{students.length === 0 && <p className="teacher-muted">暂无学生。请复制邀请码给学生。</p>}</div>
        <div className="card"><h2>最近提交</h2><div className="teacher-scroll"><table><thead><tr><th>提交时间</th><th>姓名</th><th>学号</th><th>课文</th><th>分数</th><th>批改详情</th></tr></thead><tbody>{students.flatMap((student) => (student.subs || []).map((sub) => ({ ...sub, name: student.name, studentNo: student.studentNo }))).sort((a, b) => b.at - a.at).slice(0, 100).map((sub) => <tr key={sub.jobId}><td>{date(sub.at)}</td><td>{sub.name}</td><td>{sub.studentNo}</td><td>{sub.title}</td><td>{sub.score ?? '—'}</td><td><a href={resultUrl(sub.jobId)} target="_blank" rel="noreferrer">查看结果</a></td></tr>)}</tbody></table></div></div>
      </>}</section></main>}
  </div>;
}
