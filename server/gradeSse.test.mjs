...
const PORT = 8991;     // 与本仓库其它测试端口错开（8951 是 ssrf / jobfault 在用）
const MOCK_PORT = 9896;
...
try {
  ...
  // 等假模型**确实收到请求**再断言。原来是固定 sleep(150)，在慢的 CI 机器上会误判成失败
  for (let i = 0; i < 100 && !mockState.systemText; i += 1) await sleep(50);
  ...
  if (server) {
    server.kill();
    // 等子进程真的退出再返回：进程还占着端口时，链上的下一个测试 spawn 会 bind 失败
    await new Promise((r) => {
      if (server.exitCode !== null || server.signalCode) return r();
