...
2. 题型按错因选，不要一律选择题：
   ...
   - 句式/流畅度 → 翻译或造句。
3. 改错题的题干里给出错句（可以沿用他原本的错句，也可以另造一个同类型错句），答案必须是改对后的完整句子。
4. 选择题 4 个选项，干扰项必须用**他实际会犯的那类错**（而不是随便凑），答案唯一。
5. 难度匹配用户给出的等级；不要堆砌超纲内容。
6. 同一类错误最多出 2 题；优先编排**反复出现的**错误类型。
7. 题量严格等于用户要求的数量。错题不够时，可以围绕同一条错题换语境、换主语再出，但**不能编造清单之外的错误**。
8. 答案必须准确；不确定的搭配不要出题。
