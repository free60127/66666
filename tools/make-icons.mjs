 ...
 *   icon.svg            矢量源文件本体
 *
 * 依赖 Playwright 做栅格化（复用 tools/shotter 里那份，不在项目依赖里）。
 * 没有 Playwright 时会明确报错，而不是悄悄产出一堆空文件。
 */
...
async function loadChromium() {
  ...
  for (const c of CANDIDATES) {
    try {
      const spec = c.startsWith('/') || /^[A-Za-z]:/.test(c) ? 'file:///' + c.replace(/\\/g, '/').replace(/^\//, '') : c;
      const mod = await import(spec);
      if (mod.chromium) return mod.chromium;
    } catch (e) { errs.push(`${c}: ${e.message.slice(0, 80)}`); }
  }
...
/**
 * 安卓自适应图标：mark 缩到 60% 并居中，底色铺满整个画布。
 * 系统会把图标裁成圆形/圆角/水滴等任意形状，四周约 33% 是"可能被裁掉"的区域 ——
 * 主体必须留在中间的安全区里，否则圆角一裁就切到笔画。
 */
function maskable(source, size) {
  const inner = source
    .replace(/<rect width="512" height="512" rx="115" fill="url\(#bg\)"\/>/, '<rect width="512" height="512" fill="#10222e"/>');
  return sized(inner.replace(
    '<g fill="none" stroke="#ffffff" stroke-width="46" stroke-linejoin="round">',
    '<g transform="translate(256,256) scale(0.62) translate(-256,-256)" fill="none" stroke="#ffffff" stroke-width="46" stroke-linejoin="round">',
  ), size);
}

/** 把若干 PNG 打包成 ICO（Vista+ 支持内嵌 PNG，不必用 BMP）。 */
...
for (const size of [16, 32, 48, 180, 192, 512]) {
  const buf = await shoot(sized(svg, size), size);
  raster[size] = buf;
