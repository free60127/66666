...
- **生成作业是流式的（不再干等 1–2 分钟）**：点「生成」后模型**一行一段**地往外写，结果页**边收边画** —— 先出标题/中文/原稿/整体评分与润色段落，再一句一句长出逐句解析，最后补上词汇辨析、地道习语、高级句式与加分表达；顶部常驻「正在生成：逐句解析 · 第 3 句」与「取消等待」。流式不可用时（代理缓冲 / 模型不认 `stream:true` / 中途断流）自动**用同一个任务号回退到轮询**，不重复提交、不重复计费；收尾时用服务端收敛过的完整结果覆盖屏幕上那份，所以「看到的」＝「存进历史的」
