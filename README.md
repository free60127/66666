...
教师可在「作业」发布自由题目、内置课文或班级共享课文，设置截止时间，查看每名学生的完成情况与成绩，并结束或重新开放收集。学生在「更多 → 加入教师班级」的已加入班级列表中打开作业；从该入口开始的练习会关联作业，逾期提交会标记为迟交。教师在「成绩 → 最近提交」写评语，学生重新打开结果后可看到。教师可建立、编辑、移除班级共享课文；学生可直接载入练习。创建者可按邮箱添加已注册的协作教师，协作教师可查看班级和管理教学内容。错题统计从启用本功能后上报的逐句批改中汇总类别、高频表达和涉及学生数；旧提交不会自动回填。
